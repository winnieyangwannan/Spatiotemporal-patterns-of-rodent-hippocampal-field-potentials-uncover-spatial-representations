...
load('Figure2_source_data_wake.mat');
...
load('Figure2_source_data_ripple.mat');
Frvall = Frvall;
...
%% OLE learn from running mua
obs = OB{1,2};
% Generate basis...
[~,Xrbf] = get1Dbasis('vonmises',75,rPOS{1}*2*pi,100);
...
scup = 8; rows = 10; cols = 11;
pos = 0.01:0.02:2.89; spacing = [0.01,0.01]; colormap(flipud(bone));
for ij = 1:length(example)
    ir = example(ij);
    idxts = find(ReplayEventIdx==ir);
